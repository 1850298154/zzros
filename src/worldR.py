...
#用于模拟虚拟世界中的多个无人机和目标
class World(object):
    def __init__(self, args:Dict, scan_areas_id=0) -> None: # # 加载到self._config # 并构造四个对象容器 uav_dict,target_dict,building_dict,plain # ,load_world_config(),construct_world()
        self.CONSTANTS_RADIUS_OF_EARTH = 6371000.       # meters (m)
        self.args = args
        self.output_type = -1
        self.load_world_config() # 加载到self._config  # 从配置文件world_config.yaml
        self.construct_world()   # SharedInf创建+更新  # 构造robots,building,plain,planner  #  从self._config读出map,width,length,timestep,random_seed信息-->到self成员变量
    ...
    def construct_world(self):
        ...
        self.construct_robots()  # _uav_dict,_target_dict
