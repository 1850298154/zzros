...
from .robotR import UAV, Target, Task, SharedInformation, PolygonRegion, BuildingTask
...
class InitPlanner(PlannerBase):
    ...
    def plan(self) :
        ...
        #     print(ab)
        # except Exception:
        #     # s =  traceback.format_exc() # 一行
        #     # print(s)
        #     s =  traceback.format_stack() # 多行
        #     print(*s)
        # print('self.debug_display_polygon()')
    ...
    def new_keypoint_debug_display_polygon(self):
        ...
        for i, uav in self.SharedInf.uav_dict.items():
            ...
            y_offset = 0  # 偏移量，可以根据具体情况调整
            font_size = 24  # 字体大小，可以根据具体情况调整
            font_size2 = 16  # 字体大小，可以根据具体情况调整

            # ax.text(uav.pos[0],uav.pos[1], s=f'0', ha='center', va='center', color=rb8(i%8), alpha=0.8, fontsize=font_size)
            ax.text(uav.pos[0],uav.pos[1], s=f'0', ha='center', va='center', color='k', alpha=0.8, fontsize=font_size)
            ax.text(key_point_traj[-1,0], key_point_traj[-1,1], s=str(uav.id), ha='center', va='center', color='k', alpha=0.8, fontsize=font_size2)
            # ax.text(uav.start_point[0],uav.start_point[1], s=f'1', ha='center', va='center')
    ...
    def save_init_planner(self, plt):
        ...
        fig_path =  file_name_without_extension + '/' + current_time + '.jpg'
        print('save_init_planner')
        print('fig_path')
        print(fig_path)
