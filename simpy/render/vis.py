...
class Visualizer():
    ...
    def update(self):
        ...
        map_width, map_length, map_height = 300, 300, 300
        # map_width, map_length, map_height = 100, 100, 100
        self.ax.cla()
    ...
    def draw_message_gcs_jq_uav(self, ):
        
        x_gcs_jq_uav = [0,0,0]+[0,0,0]
        # x_gcs_jq_uav = [-50,-50,-50,-50,-50,-50,]
        y_gcs_jq_uav = [0,0,0]+[0,0,0]
        # y_gcs_jq_uav = [-50,-50,-50,-50,-50,-50,]
        # z_gcs_jq_uav = list([480, 500, 520])[::-1]
        # z_gcs_jq_uav = list([480, 500, 520])[::-1]
        z_gcs_jq_uav = [520,500,480,460,440,420]
        for i in range(3) :
            role = entity.msg.msglist[i][ 0]
            msg1  = entity.msg.msglist[i][-1]
            msg2  = entity.msg.msglist[i][-2]
            self.ax.text(
                x_gcs_jq_uav[i],
                y_gcs_jq_uav[i],
                z_gcs_jq_uav[i], 
    ...
    def draw_uav(self):
        ...
        # textz = 600
        # [440, 480, 520]
        # for textz in [400 + i*20 for i in range(15) ] :
        #     self.ax.text(
        #         0,
        #         0,
        #         textz, 
        ...
        for uavid, uav in entity.uav.order2uav.items():
            if uav.task_status == protocol.uav.task_status.dead:
                self.ax.scatter(
                    uav.pos[0], 
                    uav.pos[1], 
                    uav.pos[2], 
                    color=COLOR_DICT["dead"], 
                    s=160, 
                    alpha=0.4
                    )  # 蓝色支架末端点
                continue
        ...
        # protocol.decision.decision(decision_str) == 
        
            if False:
                pass
            elif protocol.uav.role(uav.role)  == protocol.uav.role.tasker:
            ...
            elif protocol.uav.role(uav.role)  == protocol.uav.role.surveil:
                self.ax.scatter(uav.pos[0], uav.pos[1], uav.pos[2], color="b", s=160, marker="o", alpha=0.2) # 机头 左红色三角
                # plot the optical axis
                p_t = np.matmul(T, uav.optical_axis)  # [349.8465406848261, 402.07519493594015, 2.9299411962766833]
                self.ax.plot([uav.pos[0], p_t[0]], [uav.pos[1], p_t[1]], [uav.pos[2], p_t[2]], color="orange", linewidth=0.8)  # 黄色实线 垂直向下
                self.ax.scatter(p_t[0], p_t[1], p_t[2], color="orange", s=6, marker="v")  # 黄色 下三角     
                pass
            elif protocol.uav.role(uav.role)  == protocol.uav.role.relay:
                self.ax.scatter(uav.pos[0], uav.pos[1], uav.pos[2], color="g", s=160*2, marker="o", alpha=0.2) # 机头 左红色三角
                pass
            else:
                tracker.debug_tracker.instance.pf('uav.role')
                tracker.debug_tracker.instance.pf(uav.role)
            ...
            # max_sec_num = 3 + 1
            wps = uav.xyzlist_list
            wps = np.array(wps)
            if uav.scan_all_finished:
                continue
            
            self.ax.plot(
                # [uav.pos[0], task.target_pos[0]], 
                # [uav.pos[1], task.target_pos[1]], 
                # [uav.pos[2], task.target_pos[2]], 
                [uav.pos[0], wps[uav.xyzlist_index][0]], 
                [uav.pos[1], wps[uav.xyzlist_index][1]], 
                [uav.pos[2], wps[uav.xyzlist_index][2]], 
                color="navy", ls="dashed", linewidth=1)  # 当前位置-->最近目标点 == 当前需要飞过去的点 # 蓝色虚线
            self.ax.plot(
                # task.key_points[task.cur_index:task.cur_index+max_sec_num,0], 
                # task.key_points[task.cur_index:task.cur_index+max_sec_num,1], 
                # task.key_points[task.cur_index:task.cur_index+max_sec_num,2], 
                wps[ uav.xyzlist_index:,  0 ], 
                wps[ uav.xyzlist_index:,  1 ], 
                wps[ uav.xyzlist_index:,  2 ], 
                color="navy", ls="dashed", linewidth=1)  # 画出接下来的 4 段 # max_sec_num == 4
    
    def draw_target(self):
        # for target in self.SharedInf.target_dict.values():  # 9 个 目标
        for targetid, target in entity.uav.targetid_2_target.items():  # 9 个 目标
    ...
    def plot_task_info(self):
        ...
        rows = ["role", "task"]
        t2_list = list(entity.uav.order2uav.items())
        dwork = {
            0:'unassigned',
            1:'relayer',
            2:'surveiler',
        ...
        self.info_ax.table(cellText=data, rowLabels=rows, colLabels=cols, 
                           ...
                           bbox=[0.14,0,0.86,1], fontsize=18)
